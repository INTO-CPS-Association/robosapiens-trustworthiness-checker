...
pub(crate) use parsed::{
    ParsedDeclaration, ParsedExpr, ParsedExprBuilder, ParsedSpecification, SourceAscription,
};
...
pub(crate) fn parse_specification(input: &str) -> Result<ParsedSpecification, DsrvSyntaxError> {
    let mut builder = ParsedExprBuilder::with_capacity(input.len() / 4);
    ...
    ParsedSpecification::new(builder, declarations)
}
