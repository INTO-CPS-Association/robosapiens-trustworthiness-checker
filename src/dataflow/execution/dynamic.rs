...
fn compile_dynamic_plan(
    ...
    let expr = parse_expr(source.as_ref()).map_err(|error| DataflowEvalError::DynamicParse {
        expression: source.clone(),
        message: error.to_string(),
    })?;
    let mut program = if let Some((environment, typ)) = &spec.typed {
        let expr = check_expression(expr, typ, environment).map_err(|errors| {
