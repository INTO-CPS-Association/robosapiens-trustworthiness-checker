...
Choose where model output values are observed. If no output flag is selected, the CLI uses stdout. A single-destination shortcut is enough when results have one destination; use `--output-config` for explicit routing, stages, mirroring, or several destinations.
...
| MQTT | `--mqtt-output` or `--output-mqtt-file PATH` | Compact JSON envelope such as `{"value":42}`; output MQTT uses Paho. |
...
An output configuration names a default destination and a map of destinations. A secondary destination must establish its role with `partition`, legacy `variables`, `mirror: true`, or a route catalog. `variables`, `partition`, and `mirror` are mutually exclusive on one destination. `limited-null` is a bounded test sink and requires a positive `limit`.

Shared and destination-local `buffer` or `coalesce` stages can change physical batching without changing logical tick values. A writer's successful send is an admission boundary; `flush` is a later local pipeline barrier. Neither proves that a remote broker consumer has persisted or observed the message. Multi-destination output has no cross-transport transaction or rollback: one destination may observe a batch before another fails.
...
- [Output configuration reference](../reference/output-configuration.md) — use JSON5 destinations, routes, selectors, and stages.
