...
flowchart TB
    ...
    accDescr: The compiled DependencyGraph supplies fixed current edges and DynamicDependencyCollector values supply exact active edges. Scheduler retains or repairs a dependency-valid full order over stable StreamId values; a dynamic cycle is an evaluation error. ExpressionActivationState separately supplies the source order and active source set. The source order and the main ExecutionSchedule, which is the full order without active source streams, become one ScheduledExecutionPlan, which ExecutionPlan partitions into physical steps. MonitorExecution applies that plan to stable per-stream Evaluator owners so every computed stream runs exactly once.
    ...
    reconfiguration["ExpressionActivationState<br/>source_order + active source set"] --> source["Source range"]
    ...
    main --> select
    select --> plan["ScheduledExecutionPlan + ExecutionPlan"]
...
{{#include ../../assets/dataflow/dynamic-dependencies.svg}}
...
A changed source/main order causes `MonitorExecution` to select a matching cached `ExecutionPlan` or construct a new one. The execution cache retains at most four inactive plans in addition to the active plan. Selecting or evicting one moves no language state, for the reason given under [runtime ownership](runtime-ownership.md#replaceable-routing).
...
- `StreamId`, `StreamSlots`, and `StreamSet`: `src/dataflow/stream_id.rs`; fixed dependencies, source metadata, and the compiled plan: `src/dataflow/monitor_plan.rs`; active source lifecycle: `src/dataflow/expression_activation.rs`;
...
- `ExecutionPlan` selection and the four-entry inactive route cache: `src/dataflow/execution/monitor_execution/plan.rs`.
