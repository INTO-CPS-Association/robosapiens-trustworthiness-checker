...
| Feature requirements | MQTT, Redis, ROS, manual | MQTT output uses Paho and requires the `mqtt` feature (enabled by default); Redis, stdout, null, and limited-null need no optional feature; ROS requires the `ros` feature and its ROS environment/overlay; manual output is programmatic-only. |
